...
int main(int argc, char** argv)
{
  ...
  odometryPub = nh.advertise<sensor_msgs::Joy>("/dji_sdk/odometry", 10);
  ...
  if (elapsed_time > ros::Duration(0.02)) {
    ...
    if (true) {
      ...
      if (current_gps_health > 3) {
        ...
        odometria.axes.push_back(local_position.point.z);
        ...
        odometryPub.publish(odometria);

        uint8_t flag = (0x40 | 0x00 | 0x08 | 0x02 |  0x00 );
...
void local_ctrl(float &xCmd, float &yCmd, float &zCmd, float &psiCmd)
{
  uint8_t flag = (0x40 | 0x00 | 0x08 | 0x02 |  0x00 );
  sensor_msgs::Joy controllerPC;
  controllerPC.axes.push_back(xCmd);
  controllerPC.axes.push_back(yCmd);
  controllerPC.axes.push_back(zCmd);
  controllerPC.axes.push_back(psiCmd);
  controllerPC.axes.push_back(flag);
  controlPub.publish(controllerPC);
}
